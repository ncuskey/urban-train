...
│   │   └── erosion.js      # Seeding domes + height normalization (Phase 2.1)
│   ├── render/
...
* **Hydrology System** (Phase 1+): Foundation for procedural water flow and river generation
  ...
  * `src/hydrology/erosion.js`: Seeding domes and height normalization (Phase 2.1)
    * `add(start, type)`: Adds deterministic island or hill domes with BFS traversal
    * `normalizeHeights(state)`: Min-max normalization to [0,1] range
    * `getState()`: Helper to get current state with heights, cells, and samples
  ...
  * **Cell site caching**: Ensures all cells have `cell.site = {x, y}` for hydrology operations
  * **Pipeline integration**: Seeding and normalization wired after refine step with dev flags
