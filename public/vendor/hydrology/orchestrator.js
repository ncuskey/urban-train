...
import { seaLevel, defaultPoissonRadius, defaultPrecip, defaultDowncut } from "./constants.js";
 ...
 */
export function runHydrology(params) {
    ...
    const riverSegments = buildRiverSegments({
        cells,
        riversData: filtered,
        rng,
        findCellAt
    ...
    return {
        cells,
        coastIslands: coastIslands,
        coastLakes: coastLakes,
        riverSegments,
        meta: {
            riversCount,
...
function groupCount(arr) {
    const m = new Map();
    for (const a of arr)
        m.set(a.river, (m.get(a.river) ?? 0) + 1);
