...
export function poissonDisc(width, height, radius, rand) {
...
export function buildDelaunay(points) {
...
export function buildCells(points, voronoi) {
    const cells = [];
...
export function makeFindCellAt(voronoi) {
    return (x, y) => {
...
export function makeGetEdgeMidpoint(voronoi) {
    return (a, b) => {
        const ringA = voronoi.cellPolygon(a);
        const ringB = voronoi.cellPolygon(b);
        if (!ringA || !ringB) {
...
export function makeGetSharedEdge(voronoi) {
    return (a, b) => {
        const ringA = voronoi.cellPolygon(a);
        const ringB = voronoi.cellPolygon(b);
        if (!ringA || !ringB) {
