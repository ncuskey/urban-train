...
function nearestFind(cells: Cell[]): (x:number,y:number)=>number {
  return (x,y) => {
...
describe("buildRiverSegments", () => {
  it("produces bezier segments with positive widths", () => {
    const cells = mkCells();
    const riversData = [
      { river: 0, cell: 0, x: 0.2, y: 0.2, type: "source" as const },
      { river: 0, cell: 1, x: 1.0, y: 0.4, type: "course" as const },
      { river: 0, cell: 2, x: 2.1, y: 0.9, type: "course" as const },
      { river: 0, cell: 3, x: 3.0, y: 1.2, type: "estuary" as const },
    ];
