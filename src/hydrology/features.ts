 ...
 */
export function markFeatures(cells: Cell[], opts?: {
  width: number; height: number;
  nameProvider?: NameProvider; // optional naming
  cornerIndex?: number;        // starting corner index (default: cell nearest to (0,0) if water, else any sea)
}): { islands: number; lakes: number } {
  const nameProvider = opts?.nameProvider;

  // Utility BFS for a homogeneous predicate
  const flood = (startId: number, pred: (c: Cell) => boolean, kind: FeatureType, num: number) => {
    const q: number[] = [startId];
    const used = new Set<number>([startId]);
    while (q.length) {
      const id = q.shift()!;
      const c = cells[id];
      c.featureType = kind;
      c.featureNumber = num;
      if (nameProvider) c.featureName = nameProvider(kind, num);

      for (const nbId of c.neighbors) {
        if (nbId == null || used.has(nbId)) continue;
        const nb = cells[nbId];
        if (pred(nb)) {
          used.add(nbId);
          q.push(nbId);
  ...
  let oceanStart: number | null = null;
