 ...
 * Should be backed by d3-delaunay/voronoi.find(x,y) in your integration.
 */
export type FindCellAt = (x: number, y: number) => number | null;

/**
 * A point from Task 5's routing output.
 * We only need river id, x/y, type, and cell for ordering heuristics.
 */
...
export function buildRiverSegments(params: {
  ...
  for (const [riverId, pts] of groups) {
    ...
    const base: PathPoint[] = pts.map(p => ({ x: p.x, y: p.y }));
 ...
 */
function addMeanders(points: PathPoint[], rng: SeededRandom): PathPoint[] {
 ...
 */
function catmullRomToBeziers(pts: PathPoint[], alpha = 1): Omit<BezierSegment, "width" | "shadowWidth" | "riverId">[] {
  const n = pts.length;
  if (n < 2) return [];

  const beziers: Omit<BezierSegment, "width" | "shadowWidth" | "riverId">[] = [];

  // Helper: distance^alpha
  const td = (p: PathPoint, q: PathPoint) => Math.pow(Math.hypot(q.x - p.x, q.y - p.y), alpha);
