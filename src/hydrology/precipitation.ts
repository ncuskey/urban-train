 ...
 * In practice, this should be backed by d3-delaunay's voronoi.find(x,y).
 */
export type FindCellAt = (x: number, y: number) => CellId | null;
...
export function calculatePrecipitation(params: {
  ...
  width: number;
  height: number;
  precipValue: number;        // 0..10 (UI slider)
  winds: WindToggles;
  rng: SeededRandom;
  ...
  function marchRay(
    x: number, y: number,
    nextStep: (pt: {x:number;y:number}) => {x:number;y:number},
    jitter: (pt: {x:number;y:number}) => void
  ) {
  ...
  const smoothed: number[] = new Array(cells.length);
  for (const c of cells) {
